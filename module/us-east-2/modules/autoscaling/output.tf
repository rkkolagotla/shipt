locals {
  launch_configuration_id   = var.launch_configuration == "" && var.create_lc ? concat(aws_launch_configuration.this.*.id, [""])[0] : var.launch_configuration
  launch_configuration_name = var.launch_configuration == "" && var.create_lc ? concat(aws_launch_configuration.this.*.name, [""])[0] : ""

  autoscaling_group_id                        = aws_autoscaling_group.this.*.id
  autoscaling_group_name                      = aws_autoscaling_group.this.*.name
  autoscaling_group_arn                       = aws_autoscaling_group.this.*.arn
  autoscaling_group_min_size                  = aws_autoscaling_group.this.*.min_size
  autoscaling_group_max_size                  = aws_autoscaling_group.this.*.max_size
  autoscaling_group_desired_capacity          = aws_autoscaling_group.this.*.desired_capacity
  autoscaling_group_default_cooldown          = aws_autoscaling_group.this.*.default_cooldown
  autoscaling_group_health_check_grace_period = aws_autoscaling_group.this.*.health_check_grace_period
  autoscaling_group_health_check_type         = aws_autoscaling_group.this.*.health_check_type
  autoscaling_group_availability_zones        = aws_autoscaling_group.this.*.availability_zones
  autoscaling_group_vpc_zone_identifier       = aws_autoscaling_group.this.*.vpc_zone_identifier
  autoscaling_group_load_balancers            = aws_autoscaling_group.this.*.load_balancers
  autoscaling_group_target_group_arns         = aws_autoscaling_group.this.*.target_group_arns
}
